...
#ifndef __SBC_COMMON_H__
#define __SBC_COMMON_H__
...
#define SBC_ALLOCATION_METHOD_LOUDNESS  0x0         /**< SBC allocation method : Loudness 0 */
#define SBC_ALLOCATION_METHOD_SNR       0x1         /**< SBC allocation method : SNR   1   */
#define SBC_SUBBANDS_4                  0x0         /**< SBC subbands number 4            */
#define SBC_SUBBANDS_8                  0x1         /**< SBC subbands number 8            */
#define SBC_HEADER_MAX_SZIE              13         /**< SBC header max size 13 in bytes = (8+2+2+2+1+1+8+8+8*1+2*8*4)/8 */
...
typedef struct _SbcFrameHeader
{
    ...
    #else
    //little endianness
    uint32_t syncword           :8;		// 同步码，byte0  = ad
    uint32_t subband_mode       :1;	    // 子带模式，0-4， 1-8
    uint32_t allocation_method  :1;     // 分配方法，0-响度， 1-信噪比
    uint32_t channel_mode       :2;     // 声道模式，0-单声道，1-双声道，2-立体声，3-联合输出
    uint32_t block_mode         :2;     // 块数，0-4块， 1- 8块， 2-12块， 3-16块
    uint32_t sample_rate_index  :2;     // 采样率表示，0-16K  1-32K   2-44.1K   3-48K
    uint32_t bitpool            :8;		//比特池。范围是2-250。此值越大，编码产生的数据越长
    uint32_t crc_check          :8;
    #endif
}SbcFrameHeader;
