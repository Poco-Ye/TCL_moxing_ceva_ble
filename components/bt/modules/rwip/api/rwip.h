...
#include "FreeRTOS.h"
#include "task.h"
#define RWIP_TASK_SCHEDULE() taskYIELD()
#else
#define RWIP_TASK_SCHEDULE() /* feed the watchdog */
#endif
...
/**
 ****************************************************************************************
 * @brief Update the channel map base on the channel assessment
 *
 * @param[in]  channel       Channel index (range:[0..79])
 * @param[in]  peer_ch_class Pointer to Channel classification from peers
 * @param[in]  ch_map        Pointer to channel Map
 *
 * @return status            true: channel map has been updated | false: channel map has not been updated
 ****************************************************************************************
 */
bool rwip_update_ch_map_with_ch_assess_bt(uint8_t ch_idx, struct chnl_map* peer_ch_class, uint8_t* ch_map);
...
/**
 ****************************************************************************************
 * @brief Update the channel map based on the channel assessment
 *
 * @param[in]  channel       Channel index (range:[0..79])
 * @param[in]  ch_map        Pointer to channel Map
 *
