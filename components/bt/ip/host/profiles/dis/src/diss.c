                                   ...
                                   uint16_t max_length)
{
    ...
    else if (val_id < DIS_VAL_MAX)
    {
        ...
        if (p_val != NULL)
        {
            ...
            {
                status = co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, att_val_len - offset, GATT_BUFFER_TAIL_LEN);

                if(status == GAP_ERR_NO_ERROR)
                {
                    co_buf_copy_data_from_mem(p_buf, &(p_val->data[offset]), att_val_len - offset);
                }
                else
                {
    ...
    else if(p_diss_env != NULL)
    {
        ...
        if (hdl != GATT_INVALID_HDL)
        {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                // allocate value data
                p_val = (struct diss_val_elmt *) ke_malloc_user(sizeof(struct diss_val_elmt) + length, KE_MEM_PROFILE);
                if(p_val != NULL)
                {
                    p_val->value  = val_id;
                    p_val->length = length;
...
uint16_t diss_value_cfm(uint32_t token, uint8_t length, const uint8_t* p_data)
{
    ...
    if(p_diss_env != NULL)
    {
        ...
        {
            status = co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, length - offset, GATT_BUFFER_TAIL_LEN);

            if(status == GAP_ERR_NO_ERROR)
            {
                co_buf_copy_data_from_mem(p_buf, &(p_data[offset]), length - offset);
            }
            else
            {
                status = ATT_ERR_INSUFF_RESOURCE;
...
void diss_value_get_handler(uint32_t token, uint8_t val_id)
{
    ...
    p_req_ind = KE_MSG_ALLOC(DISS_VALUE_REQ_IND, PRF_DST_TASK(DISS), PRF_SRC_TASK(DISS), diss_value_req_ind);
                          ...
                          struct diss_db_cfg *p_params, const diss_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_diss_env = (diss_env_t *) ke_malloc_user(sizeof(diss_env_t), KE_MEM_PROFILE);
 ...
 ****************************************************************************************
 */
__STATIC void diss_con_create(prf_data_t *p_env, uint8_t conidx, bool is_le_con)
{
    // Nothing to do
}
