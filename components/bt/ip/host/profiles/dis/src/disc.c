...
typedef struct disc_env
{
    ...
    disc_cnx_env_t*      p_env[HOST_CONNECTION_MAX];
                          ...
                          uint8_t nb_att, const gatt_svc_att_t* p_atts)
{
    ...
    if(p_disc_env != NULL)
    {
        ASSERT_INFO(p_disc_env->p_env[conidx] != NULL, conidx, user_lid);
...
__STATIC void disc_att_val_cb(uint8_t conidx, uint8_t user_lid, uint16_t dummy, uint16_t hdl, uint16_t offset,
                              co_buf_t* p_data)
{
    disc_read_val_cmp(conidx, GAP_ERR_NO_ERROR, (uint8_t) dummy, co_buf_data_len(p_data), co_buf_data(p_data));
}
...
uint16_t disc_enable(uint8_t conidx, uint8_t con_type, const disc_dis_content_t* p_dis)
{
    ...
    if(p_disc_env != NULL)
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_disc_env->p_env[conidx] == NULL))
        {
            // allocate environment variable for task instance
            p_disc_env->p_env[conidx] = (disc_cnx_env_t *) ke_malloc_user(sizeof(disc_cnx_env_t), KE_MEM_PROFILE);
...
uint16_t disc_read_val(uint8_t conidx, uint8_t val_id)
{
    ...
    if(p_disc_env != NULL)
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_disc_env->p_env[conidx] != NULL) && (p_disc_env->p_env[conidx]->state == DISC_IDLE))
        {
                          ...
                          const void* p_params, const disc_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_disc_env = (disc_env_t*) ke_malloc_user(sizeof(disc_env_t), KE_MEM_PROFILE);
 ...
 ****************************************************************************************
 */
__STATIC void disc_con_create(prf_data_t *p_env, uint8_t conidx, bool is_le_con)
{
    // Nothing to do
}
