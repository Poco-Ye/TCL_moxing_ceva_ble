...
typedef struct basc_env
{
    ...
    basc_cnx_env_t*      p_env[HOST_CONNECTION_MAX];
...
__STATIC uint16_t basc_read_val(uint8_t conidx, uint8_t bas_instance, uint16_t val_id)
{
    ...
    if(p_basc_env != NULL)
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_basc_env->p_env[conidx] != NULL) && (!p_basc_env->p_env[conidx]->discover))
        {
            basc_cnx_env_t* p_con_env = p_basc_env->p_env[conidx];
...
uint16_t basc_enable(uint8_t conidx, uint8_t con_type, uint8_t nb_bas, const bas_content_t* p_bas)
{
    ...
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_basc_env->p_env[conidx] == NULL))
        {
            // allocate environment variable for task instance
            p_basc_env->p_env[conidx] = (basc_cnx_env_t *) ke_malloc_user(sizeof(basc_cnx_env_t), KE_MEM_PROFILE);
                          ...
                          const void* p_params, const basc_cb_t* p_cb)
{
    ...
    do
    {
        ...
        if(   (p_cb == NULL) || (p_cb->cb_enable_cmp == NULL)
        ...
        p_basc_env = (basc_env_t*) ke_malloc_user(sizeof(basc_env_t), KE_MEM_PROFILE);
