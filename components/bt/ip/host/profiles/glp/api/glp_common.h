...
union glp_filter
{
    ...
    {
        /// Min base time
        prf_date_time_t facetime_min;

        /// Max base time
        prf_date_time_t facetime_max;
    } time;
};


/// Record access control point request
typedef struct glp_racp_req
{
    /// operation code (see enum #glp_racp_op_code)
    uint8_t           op_code;
    /// function operator (see enum #glp_racp_operator)
    uint8_t           operator;
    /// filter type (see enum #glp_racp_filter)
    uint8_t           filter_type;
