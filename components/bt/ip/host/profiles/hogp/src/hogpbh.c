...
typedef struct hogpbh_env
{
    ...
    hogpbh_cnx_env_t*    p_env[HOST_CONNECTION_MAX];
 ...
 ****************************************************************************************
 */
__STATIC void hogpbh_read_val_cmp(uint8_t conidx, uint16_t status, uint16_t dummy, co_buf_t* p_data)
{
...
__STATIC uint16_t hogpbh_read_val(uint8_t conidx, uint8_t hid_idx, uint16_t val_id)
{
    ...
    if(p_hogpbh_env != NULL)
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_hogpbh_env->p_env[conidx] != NULL) && (!p_hogpbh_env->p_env[conidx]->discover))
        {
            hogpbh_cnx_env_t* p_con_env = p_hogpbh_env->p_env[conidx];
...
__STATIC uint16_t hogpbh_write_val(uint8_t conidx, uint8_t hid_idx, uint8_t val_id, bool wr_cmd, co_buf_t* p_buf)
{
    uint16_t status = PRF_ERR_REQ_DISALLOWED;
    // Client environment
    hogpbh_env_t* p_hogpbh_env = PRF_ENV_GET(HOGPBH, hogpbh);
...
uint16_t hogpbh_enable(uint8_t conidx, uint8_t con_type, uint8_t hids_nb, const hogpbh_content_t* p_hids)
{
    ...
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_hogpbh_env->p_env[conidx] == NULL))
        {
            // allocate environment variable for task instance
            p_hogpbh_env->p_env[conidx] = (hogpbh_cnx_env_t *) ke_malloc_user(sizeof(hogpbh_cnx_env_t), KE_MEM_PROFILE);
...
uint16_t hogpbh_write_report(uint8_t conidx, uint8_t hid_idx, uint8_t val_id, bool wr_cmd, co_buf_t* p_report)
{
    uint16_t status;
 ...
 ****************************************************************************************
 */
__STATIC int hogpbh_enable_req_handler(ke_msg_id_t const msgid, struct hogpbh_enable_req const *p_param,
                                       ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogpbh_enable_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_ENABLE_RSP, src_id, dest_id, hogpbh_enable_rsp);
        if(p_rsp != NULL)
        {
 ...
 ****************************************************************************************
 */
__STATIC int hogpbh_read_info_req_handler(ke_msg_id_t const msgid, struct hogpbh_read_info_req const *p_param,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    uint8_t status;
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogpbh_read_info_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_READ_INFO_RSP, src_id, dest_id, hogpbh_read_info_rsp);
 ...
 ****************************************************************************************
 */
__STATIC int hogpbh_write_req_handler(ke_msg_id_t const msgid, struct hogpbh_write_req const *p_param,
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    uint8_t status;
    ...
    switch (p_param->info)
    {
        ...
        case HOGPBH_BOOT_MOUSE_IN_REPORT:
        {
            ...
            {
                co_buf_copy_data_from_mem(p_buf, p_param->data.boot_report.value, report_len);
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogpbh_write_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_WRITE_RSP, src_id, dest_id, hogpbh_write_rsp);
...
{
    struct hogpbh_enable_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_ENABLE_RSP, PRF_DST_TASK(HOGPBH), PRF_SRC_TASK(HOGPBH),
                                                   hogpbh_enable_rsp);
...
{
    struct hogpbh_read_info_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_READ_INFO_RSP, PRF_DST_TASK(HOGPBH), PRF_SRC_TASK(HOGPBH),
 ...
 ****************************************************************************************
 */
__STATIC void hogpbh_cb_read_report_cmp(uint8_t conidx, uint16_t status, uint8_t hid_idx, uint8_t val_id, co_buf_t* p_report)
{
    uint16_t data_len = co_buf_data_len(p_report);
    struct hogpbh_read_info_rsp *p_rsp = KE_MSG_ALLOC_DYN(HOGPBH_READ_INFO_RSP, PRF_DST_TASK(HOGPBH), PRF_SRC_TASK(HOGPBH),
    ...
    if(p_rsp != NULL)
    {
        ...
        co_buf_copy_data_to_mem(p_report, p_rsp->data.boot_report.value, data_len);
...
{
    struct hogpbh_write_rsp *p_rsp = KE_MSG_ALLOC(HOGPBH_WRITE_RSP, PRF_DST_TASK(HOGPBH), PRF_SRC_TASK(HOGPBH),
                                                   hogpbh_write_rsp);
 ...
 ****************************************************************************************
 */
__STATIC void hogpbh_cb_report(uint8_t conidx, uint8_t hid_idx, uint8_t val_id, co_buf_t* p_report)
{
    uint16_t data_len = co_buf_data_len(p_report);
    // send boot report indication
    struct hogpbh_boot_report_ind *p_ind = KE_MSG_ALLOC_DYN(HOGPBH_BOOT_REPORT_IND, PRF_DST_TASK(HOGPBH),
    ...
    if(p_ind != NULL)
    {
        ...
        co_buf_copy_data_to_mem(p_report, p_ind->boot_report.value, data_len);
                            ...
                            const void* p_params, const hogpbh_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_hogpbh_env = (hogpbh_env_t*) ke_malloc_user(sizeof(hogpbh_env_t), KE_MEM_PROFILE);
...
__STATIC uint16_t hogpbh_destroy(prf_data_t *p_env, uint8_t reason)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        for (idx = 0; idx < HOST_CONNECTION_MAX; idx++)
        {
            ...
            {
                ke_free(p_hogpbh_env->p_env[idx]);
            }
        }

        // free profile environment variables
        p_env->p_env = NULL;
        ke_free(p_hogpbh_env);
    }
 ...
 ****************************************************************************************
 */
__STATIC void hogpbh_con_create(prf_data_t *p_env, uint8_t conidx, bool is_le_con)
{
    // Nothing to do
}
...
__STATIC void hogpbh_con_cleanup(prf_data_t *p_env, uint8_t conidx, uint16_t reason)
{
    ...
    if (p_hogpbh_env->p_env[conidx] != NULL)
    {
        ke_free(p_hogpbh_env->p_env[conidx]);
        p_hogpbh_env->p_env[conidx] = NULL;
    }
}
