...
typedef struct hogprh_env
{
    ...
    hogprh_cnx_env_t*    p_env[HOST_CONNECTION_MAX];
 ...
 ****************************************************************************************
 */
__STATIC void hogprh_read_val_cmp(uint8_t conidx, uint16_t status, uint16_t dummy, co_buf_t* p_data)
{
    ...
    if(p_hogprh_env != NULL)
    {
        ...
        switch(val_id)
        {
            ...
            case HOGPRH_PROTO_MODE:
            {
                ...
                {
                    proto_mode = co_buf_data(p_data)[0];
                }
            ...
            case HOGPRH_HID_INFO:
            {
                ...
                if(status == CO_BUF_ERR_NO_ERROR)
                {
                    hid_info.bcdHID       = co_btohs(co_read16p(co_buf_data(p_data)));
                    co_buf_head_release(p_data, 2);
                    hid_info.bCountryCode = co_buf_data(p_data)[0];
                    co_buf_head_release(p_data, 1);
                    hid_info.flags        = co_buf_data(p_data)[0];
                    co_buf_head_release(p_data, 1);
                }
            ...
            case HOGPRH_REPORT_MAP_EXT_REP_REF:
            {
                ...
                if(status == CO_BUF_ERR_NO_ERROR)
                {
                    uint8_t uuid_len = co_buf_data_len(p_data);
                    ...
                    {
                        co_buf_copy_data_to_mem(p_data, report_map_ref.uuid, uuid_len);
            ...
            case HOGPRH_REPORT_REF:
            {
                ...
                if(status == CO_BUF_ERR_NO_ERROR)
                {
                    report_ref.id    = co_buf_data(p_data)[0];
                    co_buf_head_release(p_data, 1);
                    report_ref.type  = co_buf_data(p_data)[0];
                    co_buf_head_release(p_data, 1);
                }
...
__STATIC uint16_t hogprh_read_val(uint8_t conidx, uint8_t hid_idx, uint16_t val_id, uint8_t report_idx)
{
    ...
    if(p_hogprh_env != NULL)
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_hogprh_env->p_env[conidx] != NULL) && (!p_hogprh_env->p_env[conidx]->discover))
        {
            hogprh_cnx_env_t* p_con_env = p_hogprh_env->p_env[conidx];
...
uint16_t hogprh_enable(uint8_t conidx, uint8_t con_type, uint8_t hids_nb, const hogprh_content_t* p_hids)
{
    ...
    {
        if ((conidx < HOST_CONNECTION_MAX) && (p_hogprh_env->p_env[conidx] == NULL))
        {
            // allocate environment variable for task instance
            p_hogprh_env->p_env[conidx] = (hogprh_cnx_env_t *) ke_malloc_user(sizeof(hogprh_cnx_env_t), KE_MEM_PROFILE);
...
uint16_t hogprh_write_proto_mode(uint8_t conidx, uint8_t hid_idx, uint8_t proto_mode)
{
    ...
    {
        co_buf_data(p_buf)[0] = proto_mode;
...

{
    ...
    else
    {
        ...
        {
            co_write16p(co_buf_data(p_buf), co_htobs(cfg_val));
...
uint16_t hogprh_write_report(uint8_t conidx, uint8_t hid_idx, uint8_t report_idx, bool wr_cmd, co_buf_t* p_report)
{
 ...
 ****************************************************************************************
 */
__STATIC int hogprh_enable_req_handler(ke_msg_id_t const msgid, struct hogprh_enable_req const *p_param,
                                       ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogprh_enable_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_ENABLE_RSP, src_id, dest_id, hogprh_enable_rsp);
        if(p_rsp != NULL)
        {
 ...
 ****************************************************************************************
 */
__STATIC int hogprh_read_info_req_handler(ke_msg_id_t const msgid, struct hogprh_read_info_req const *p_param,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    uint8_t status;
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogprh_read_info_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_READ_INFO_RSP, src_id, dest_id, hogprh_read_info_rsp);
 ...
 ****************************************************************************************
 */
__STATIC int hogprh_write_req_handler(ke_msg_id_t const msgid, struct hogprh_write_req const *p_param,
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    uint8_t status;
    ...
    switch (p_param->info)
    {
        ...
        case HOGPRH_REPORT:
        {
            ...
            // allocate a buffer to perform write
            if(co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, report_len, GATT_BUFFER_TAIL_LEN) == CO_BUF_ERR_NO_ERROR)
            {
                co_buf_copy_data_from_mem(p_buf, p_param->data.report.value, report_len);
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        struct hogprh_write_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_WRITE_RSP, src_id, dest_id, hogprh_write_rsp);
...
{
    struct hogprh_enable_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_ENABLE_RSP, PRF_DST_TASK(HOGPRH), PRF_SRC_TASK(HOGPRH),
                                                   hogprh_enable_rsp);
...
{
    struct hogprh_read_info_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_READ_INFO_RSP, PRF_DST_TASK(HOGPRH), PRF_SRC_TASK(HOGPRH),
 ...
 ****************************************************************************************
 */
__STATIC void hogprh_cb_read_report_map_cmp(uint8_t conidx, uint16_t status, uint8_t hid_idx, co_buf_t* p_report_map)
{
    uint16_t data_len = co_buf_data_len(p_report_map);
    struct hogprh_read_info_rsp *p_rsp = KE_MSG_ALLOC_DYN(HOGPRH_READ_INFO_RSP, PRF_DST_TASK(HOGPRH), PRF_SRC_TASK(HOGPRH),
    ...
    if(p_rsp != NULL)
    {
        ...
        co_buf_copy_data_to_mem(p_report_map, p_rsp->data.report_map.value, data_len);
                                        ...
                                        co_buf_t* p_report)
{
    ...
    if(p_rsp != NULL)
    {
        ...
        co_buf_copy_data_to_mem(p_report, p_rsp->data.report.value, data_len);
...
{
    struct hogprh_write_rsp *p_rsp = KE_MSG_ALLOC(HOGPRH_WRITE_RSP, PRF_DST_TASK(HOGPRH), PRF_SRC_TASK(HOGPRH),
                                                   hogprh_write_rsp);
 ...
 ****************************************************************************************
 */
__STATIC void hogprh_cb_report(uint8_t conidx, uint8_t hid_idx, uint8_t report_idx, co_buf_t* p_report)
{
    uint16_t data_len = co_buf_data_len(p_report);
    // send report indication
    struct hogprh_report_ind *p_ind = KE_MSG_ALLOC_DYN(HOGPRH_REPORT_IND, PRF_DST_TASK(HOGPRH),
    ...
    if(p_ind != NULL)
    {
        ...
        co_buf_copy_data_to_mem(p_report, p_ind->report.value, data_len);
                            ...
                            const void* p_params, const hogprh_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_hogprh_env = (hogprh_env_t*) ke_malloc_user(sizeof(hogprh_env_t), KE_MEM_PROFILE);
...
__STATIC uint16_t hogprh_destroy(prf_data_t *p_env, uint8_t reason)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        for (idx = 0; idx < HOST_CONNECTION_MAX; idx++)
        {
 ...
 ****************************************************************************************
 */
__STATIC void hogprh_con_create(prf_data_t *p_env, uint8_t conidx, bool is_le_con)
{
    // Nothing to do
}
