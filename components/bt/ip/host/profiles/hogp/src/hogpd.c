...
typedef struct hogpd_env
{
    ...
    uint16_t        ntf_cfg[HOGPD_NB_HIDS_INST_MAX][HOST_CONNECTION_MAX];
    ...
    for (*p_svc_idx = 0; (*p_svc_idx < p_hogpd_env->hids_nb) && (handle >= hdl_cursor); (*p_svc_idx)++)
    {
        ...
        *p_report_idx = 0;
        ...
        else
        {
            ...
            // not expected
            ASSERT_ERR(*p_att_idx != HOGPD_IDX_NB);
        }
...
__STATIC void hogpd_exe_operation(hogpd_env_t* p_hogpd_env)
{
    if(!p_hogpd_env->in_exe_op)
    {
        ...
        while(!co_list_is_empty(&(p_hogpd_env->wait_queue)) && !(p_hogpd_env->op_ongoing))
        {
            uint16_t status = GAP_ERR_NO_ERROR;
            co_buf_t* p_buf = (co_buf_t*) co_list_pop_front(&(p_hogpd_env->wait_queue));
            hogpd_buf_meta_t* p_meta = (hogpd_buf_meta_t*) co_buf_metadata(p_buf);
   ...
   do
    {
        ...
        switch (att_idx)
        {
            ...
            default:
            {
                ...
                else if(co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, 0, L2CAP_LE_MTU_MIN + GATT_BUFFER_HEADER_LEN) != CO_BUF_ERR_NO_ERROR)
                {
        ...
        switch (att_idx)
        {
            ...
            {
                co_buf_tail(p_buf)[0] = p_hogpd_env->svcs[hid_idx].proto_mode;
            ...
            case HOGPD_IDX_HID_INFO_VAL:
            {
                ...
                co_write16p(co_buf_tail(p_buf), co_htobs(p_hid_info->bcdHID));
                co_buf_tail_reserve(p_buf, 2);
                co_buf_tail(p_buf)[0] = p_hid_info->bCountryCode;
                co_buf_tail_reserve(p_buf, 1);
                co_buf_tail(p_buf)[0] = p_hid_info->flags;
                co_buf_tail_reserve(p_buf, 1);
            } break;

            case HOGPD_IDX_REPORT_MAP_EXT_REP_REF:
            {
                co_write16p(co_buf_tail(p_buf), co_htobs(p_hogpd_env->svcs[hid_idx].rep_ref_uuid));
                co_buf_tail_reserve(p_buf, 2);
            } break;
            ...
            {
                // Pack report ID
                co_buf_tail(p_buf)[0] = p_hogpd_env->svcs[hid_idx].report_id[report_idx];
                co_buf_tail_reserve(p_buf, 1);
                // Pack report Type
                co_buf_tail(p_buf)[0] = (p_hogpd_env->svcs[hid_idx].report_char_cfg[report_idx] & HOGPD_CFG_REPORT_FEAT);
                co_buf_tail_reserve(p_buf, 1);
            } break;
                                   ...
                                   co_buf_t* p_data)
{
    ...
    do
    {
        ...
        switch (att_idx)
        {
            ...
            {
                uint8_t ctnl_pt_val = co_buf_data(p_data)[0];
            ...
            case HOGPD_IDX_PROTO_MODE_VAL:
            {
                uint8_t proto_mode = co_buf_data(p_data)[0];
...
uint16_t hogpd_report_upd(uint8_t conidx, uint8_t hid_idx, uint8_t report_idx, uint8_t report_type, co_buf_t* p_buf)
{
    ...
    else if((p_hogpd_env != NULL) && (gapc_is_estab(conidx)))
    {
        uint16_t hdl;
        ...
        else if (co_buf_data_len(p_buf) > max_report_len)
        {
        ...
        else
        {
            ...
            hogpd_buf_meta_t* p_buf_meta = (hogpd_buf_meta_t*) co_buf_metadata(p_buf);
            ...
            co_list_push_back(&(p_hogpd_env->wait_queue), &(p_buf->hdr));
            // execute operation
            hogpd_exe_operation(p_hogpd_env);
        }
    }
 ...
 ****************************************************************************************
 */
__STATIC int hogpd_enable_req_handler(ke_msg_id_t const msgid, struct hogpd_enable_req const *p_param,
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    // Send back response
    p_rsp = KE_MSG_ALLOC(HOGPD_ENABLE_RSP, src_id, dest_id, hogpd_enable_rsp);
    if(p_rsp != NULL)
    {
 ...
 ****************************************************************************************
 */
__STATIC int hogpd_report_upd_req_handler(ke_msg_id_t const msgid, struct hogpd_report_upd_req const *p_param,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    uint16_t status;
    co_buf_t* p_buf = NULL;
    ...
    if(co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, p_param->report.length, GATT_BUFFER_TAIL_LEN) == CO_BUF_ERR_NO_ERROR)
    {
        co_buf_copy_data_from_mem(p_buf, p_param->report.value, p_param->report.length);
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        ...
        struct hogpd_report_upd_rsp *p_rsp = KE_MSG_ALLOC(HOGPD_REPORT_UPD_RSP, src_id, dest_id, hogpd_report_upd_rsp);
        if(p_rsp != NULL)
        {
            p_rsp->conidx = p_param->conidx;
            p_rsp->status = status;
 ...
 ****************************************************************************************
 */
__STATIC int hogpd_report_read_cfm_handler(ke_msg_id_t const msgid, struct hogpd_report_read_cfm const *p_param,
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    co_buf_t* p_buf = NULL;
    ...
    // allocate report buffer
    if(   (status == GAP_ERR_NO_ERROR)
       && co_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, p_param->report.length, GATT_BUFFER_TAIL_LEN) == CO_BUF_ERR_NO_ERROR)
    {
        co_buf_copy_data_from_mem(p_buf, p_param->report.value, p_param->report.length);
    }
    else
 ...
 ****************************************************************************************
 */
__STATIC int hogpd_report_write_cfm_handler(ke_msg_id_t const msgid, struct hogpd_report_write_cfm const *p_param,
                                            ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
__STATIC int hogpd_proto_mode_cfm_handler(ke_msg_id_t const msgid, struct hogpd_proto_mode_cfm const *p_param,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
...
KE_MSG_HANDLER_TAB(hogpd)
{
    ...
    { HOGPD_REPORT_READ_CFM,         (ke_msg_func_t) hogpd_report_read_cfm_handler  },
    { HOGPD_REPORT_WRITE_CFM,        (ke_msg_func_t) hogpd_report_write_cfm_handler },
...
{
    struct hogpd_ntf_cfg_ind* p_ind =  KE_MSG_ALLOC(HOGPD_NTF_CFG_IND, PRF_DST_TASK(HOGPD),
...
{
    struct hogpd_proto_mode_req_ind* p_req_ind =  KE_MSG_ALLOC(HOGPD_PROTO_MODE_REQ_IND,
...
{
    struct hogpd_report_read_req_ind* p_req_ind =  KE_MSG_ALLOC(HOGPD_REPORT_READ_REQ_IND,
                                                                PRF_DST_TASK(HOGPD), PRF_SRC_TASK(HOGPD),
...
__STATIC void hogpd_cb_report_write_req(uint8_t conidx, uint16_t token, uint8_t hid_idx, uint8_t report_idx,
                                        uint8_t report_type, co_buf_t* p_buf)
{
    uint16_t data_len = ((p_buf == NULL) ? 0 : co_buf_data_len(p_buf));
    struct hogpd_report_write_req_ind* p_req_ind =  KE_MSG_ALLOC_DYN(HOGPD_REPORT_WRITE_REQ_IND,
                                                                     PRF_DST_TASK(HOGPD), PRF_SRC_TASK(HOGPD),
    ...
    if(p_req_ind != NULL)
    {
        ...
        if(data_len > 0)
        {
            co_buf_copy_data_to_mem(p_buf, p_req_ind->report.value, data_len);
        }
        ke_msg_send(p_req_ind);
    }
}
...
{
    struct hogpd_ctnl_pt_ind* p_ind =  KE_MSG_ALLOC(HOGPD_CTNL_PT_IND, PRF_DST_TASK(HOGPD),
...
{
    struct hogpd_report_upd_rsp* p_rsp =  KE_MSG_ALLOC(HOGPD_REPORT_UPD_RSP, PRF_DST_TASK(HOGPD),
                          ...
                          struct hogpd_db_cfg *p_params, const hogpd_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_hogpd_env = (hogpd_env_t *) ke_malloc_user(sizeof(hogpd_env_t), KE_MEM_PROFILE);
        ...
        for (svc_idx = 0; ((svc_idx < p_params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
        {
            ...
            p_hids_db[svc_idx] = (gatt_att16_desc_t *) ke_malloc_user(HOGPD_ATT_MAX * sizeof(gatt_att16_desc_t), KE_MEM_NON_RETENTION);
        ...
        co_list_init(&(p_hogpd_env->wait_queue));
...
__STATIC uint16_t hogpd_destroy(prf_data_t *p_env, uint8_t reason)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        if(reason != PRF_DESTROY_RESET)
        {
            ...
            while(!co_list_is_empty(&p_hogpd_env->wait_queue))
            {
                co_buf_t* p_buf = (co_buf_t*) co_list_pop_front(&p_hogpd_env->wait_queue);
                co_buf_release(p_buf);
            }
 ...
 ****************************************************************************************
 */
__STATIC void hogpd_con_create(prf_data_t *p_env, uint8_t conidx, bool is_le_con)
{
    // Nothing to do
}
