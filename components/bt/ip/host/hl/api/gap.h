...
enum gap_ad_type_flag
{
    ...
    /// Dual mode for host supported (BR/EDR/LE) - AD Flag - bit mask
    GAP_SIMUL_BR_EDR_LE_HOST_BIT        = 0x10,
    /// Dual mode for host supported (BR/EDR/LE) - AD Flag - bit position
...
#define GAP_EIR_MAX_DATA_SIZE                               (240)
...
typedef struct gap_bdaddr
{
    /// BD Address of device
    uint8_t addr[GAP_BD_ADDR_LEN];
    /// Address type of the device 0=public/1=private random (see enum #gap_addr_type)
    uint8_t addr_type;
} gap_bdaddr_t;
...
typedef struct gap_ral_dev_info
{
    /// Device identity
    gap_bdaddr_t addr;
    /// Privacy Mode (see enum #gap_privacy_mode)
    uint8_t      priv_mode;
    /// Peer IRK
...
typedef struct gap_aes_rand_nb
{
    /// AES cipher result value
    uint8_t value[GAP_AES_LEN];
} gap_aes_rand_nb_t;
