...
#include "htpt.h"               // health thermometer functions
#include "co_bt.h"
#include "prf_types.h"
#include "prf_utils.h"
...
void app_timer_ht_handler(void* p_env)
   {
   
      //uint16_t htpt_data_send(const uint8_t* data, uint8_t len,bool ind_noti);
      //uint8_t data[]={0x12,0x34,0x56,0x78,0xab,0xcd,0xef,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99};

		dbg_print2("app_timer_ht_handler\r\n");
      //htpt_data_send(data,16,1);	
...
static void app_ht_temp_send(void)
{
    ...
    struct htpt_temp_send_req * req = KE_MSG_ALLOC(HTPT_TEMP_SEND_REQ,
                                                    prf_get_task_num_from_api_id(TASK_ID_HTPT),
                                                    TASK_APP,
                                                    htpt_temp_send_req);
...
//    req->temp_meas.time_stamp = 0;
    ...
    //SETB(req->temp_meas.flags, HTP_FLAG_TYPE, 1); liujin del
...
//add test send temp 
void test_send_temp(void)
	{
		 // Temperature Value
		 int32_t value = APP_HT_TEMP_VALUE_INIT;//(int32_t)(app_ht_env.temp_value);
	...
	// 	req->temp_meas.time_stamp = 0;
		 req->temp_meas.flags		  = 0;
		 //SETB(req->temp_meas.flags, HTP_FLAG_TYPE, 1); liujin del
		 req->temp_meas.type 		  = 1; //app_ht_env.temp_meas_type;
...
void app_ht_add_hts(void)
{
    ...
    req->sec_lvl     = SVC_SEC_LVL(NO_AUTH); //req->sec_lvl = PERM(SVC_AUTH, AUTH);
    req->prf_api_id = TASK_ID_HTPT;
...
const struct app_subtask_handlers app_ht_handlers = {&app_ht_msg_handler_list[0], ARRAY_LEN(app_ht_msg_handler_list)};
