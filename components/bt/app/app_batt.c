...
#include "bass.h"               // health thermometer functions
#include "co_bt.h"
#include "co_utils.h"
...
void app_timer_batt_handler(void* p_env)
   {
   uint32_t adc_data;
   uint8_t level_num = 0;
   adc_data = aux_adc_top_get_data();
	//dbg_print("adc_data:%x\r\n",adc_data);
	level_num = adc_data/100;
	if (level_num > 100)
		level_num = 100;
   dbg_print("*adc_data:%x,level_num:%x\r\n",adc_data,level_num);
	app_batt_send_lvl(level_num);
	if (app_get_connection_state())
   	co_time_timer_set(&(app_env.batt_timer), 100000); 
...
void app_batt_add_bas(void)
{
    ...
    req->sec_lvl     = SVC_SEC_LVL(NO_AUTH); //PERM(SVC_AUTH, NO_AUTH); gatt_sec_lvl,SVC_SEC_LVL(NO_AUTH)
    ...
    db_cfg->features[0] = BAS_BATT_LVL_NTF_SUP;
	 co_time_timer_init(&(app_env.batt_timer), app_timer_batt_handler, NULL);
...
void app_batt_enable_prf(uint8_t conidx)
{
    ...
    struct bass_enable_req * req = KE_MSG_ALLOC(BASS_ENABLE_REQ,
                                               ...
                                               prf_get_task_num_from_api_id(TASK_ID_BASS),
                                                TASK_APP,
                                                bass_enable_req);
    ...
    req->ntf_cfg           = PRF_CLI_START_NTF;//PRF_CLI_STOP_NTFIND; 
    req->old_batt_lvl[0]   = 50; 

    // Send the message
    ke_msg_send(req);

    //add test batt notify
    {
    uint32_t adc_data;
    uint8_t level_num = 0;
    adc_data = aux_adc_top_get_data();
    ...
    dbg_print("*adc_data:%x,level_num:%x\r\n",adc_data,level_num);
    app_batt_send_lvl(level_num);  
    co_time_timer_set(&(app_env.batt_timer), 30000); 
    }
...
static int batt_level_upd_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    //dbg_print("batt_level_upd_handler:%x\r\n",param->status);
    return (KE_MSG_CONSUMED);
}
...
const struct app_subtask_handlers app_batt_handlers = {&app_batt_msg_handler_list[0], ARRAY_LEN(app_batt_msg_handler_list)};
