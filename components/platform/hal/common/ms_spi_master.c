 ...
 * @retval None
 */
void ms_spi_master_enable_cpu_interrupt(SpiMasterHandle_Type *spi) {
       CHECK_PTR_NULL(spi);
 ...
 * @retval None
 */
void ms_spi_master_disable_cpu_interrupt(SpiMasterHandle_Type *spi) { 
       CHECK_PTR_NULL(spi);
 ...
 */
int32_t ms_spi_master_init(SpiMasterHandle_Type *spi)
{
 ...
 * @retval STATUS_SUCCESS or STATUS_ERROR
 */
int32_t ms_spi_master_deinit(SpiMasterHandle_Type *spi)
{
     CHECK_PTR_NULL_RET(spi, STATUS_ERROR);
 ...
 * @retval STATUS_SUCCESS or STATUS_ERROR
 */
int32_t ms_spi_master_send(SpiMasterHandle_Type *spi, const uint8_t *data, uint16_t size, uint32_t timeout)
{
 ...
 * @retval STATUS_SUCCESS or STATUS_ERROR
 */
int32_t ms_spi_master_receive(SpiMasterHandle_Type *spi, uint8_t *rx_data, uint16_t size, uint32_t timeout)
{
    int reg_value = 0;
 ...
 * @retval STATUS_SUCCESS or STATUS_ERROR
 */
int32_t ms_spi_master_int_send(SpiMasterHandle_Type *spi, const uint8_t *data, uint16_t size, uint32_t timeout)
{
    int32_t reg_value = 0;
     ...
     if(spi->interrupt.status == SPI_MASTER_STATUS_BUSY)
     {
          return -1;
      }
   ...
   ms_spi_set_transmit_fifo_threshold_level_hal(spi->instance,0);
    ...
    if( spi->interrupt.tx_total_len>0)
    {
        ...
        do
        {
            ...
            /*set  busy  flag*/
          spi->interrupt.status = SPI_MASTER_STATUS_BUSY;
    ...
    while(spi->interrupt.status == SPI_MASTER_STATUS_BUSY);
 ...
 * @retval STATUS_SUCCESS or STATUS_ERROR
 */
int32_t ms_spi_master_int_receive(SpiMasterHandle_Type *spi, uint8_t *rx_data, uint16_t size, uint32_t timeout)
{
    int reg_value = 0;
...
void spi_master_int_fifo_empty(SpiMasterHandle_Type *spi,uint8_t status)
{
  ...
  if(status&SPI_ISR_TXEIS)
  {
     ...
     else
     {
	       ...
	       if( spi->interrupt.tx_len ==  spi->interrupt.tx_total_len)
	     {
             ...
             /*not busy*/
             spi->interrupt.status = SPI_MASTER_STATUS_IDLE;
...
void spi_master_int_fifo_full(SpiMasterHandle_Type *spi,uint8_t status)
{
   int32_t remain_len = 0;
   int32_t wr_cnt = 0;
   int32_t reg_value = 0;
 ...
 * @retval None
 */
void ms_spi_master_irq_handler(SpiMasterHandle_Type *spi) {
    int32_t reg_value = 0;
    int32_t send_len = 0;
    int32_t wr_cnt = 0;
    ...
    case SPI_ISR_TXEIS:
	 ...
	 spi_master_int_fifo_empty(spi,SPI_ISR_TXEIS);
	...
	spi_master_int_fifo_full(spi,SPI_ISR_RXFIS);
  ...
  if (spi->p_callback == NULL) {
		spi->error_code |= SPI_MASTER_ERROR_INVALID_CALLBACK;
		return;
	}
